...
let fullOverlapSets = 0
for (let pair = 0; pair < elfPairs.length; pair++) {
    const [[elf1Start, elf1End], [elf2Start, elf2End]] =
        elfPairs[pair].split(',').map(range => range.split('-').map(i=>Number(i)))
...
console.log(`Number of pairs with fully overlapping ranges: ${fullOverlapSets}`)